...
def train_fit_models(labels, df_x):
    ...
    df_x.to_csv(f'./excel_files/{filename}/churn.csv', index=False)
...
def print_metrics(X_test, Y_test, pred_test, model):
    ...
    print(conf_matrix)
    print('Recall for the test data;', str(recall_score(Y_test, pred_test) * 100) + '%')
    print('Accuracy for the test data: ', str(model.score(np.array(X_test), np.array(Y_test)) * 100) + '%')
    print('Precision for the test data;', str(precision_score(Y_test, pred_test) * 100) + '%')
    tn, fp, fn, tp = conf_matrix.ravel()
    fp_rate = float(fp) / float(fp + tn) * 100
    print('The False positive rate of test data;', fp_rate)
    print('F score for the test data;', str(f1_score(Y_test, pred_test) * 100) + '%')
